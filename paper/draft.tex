...
\title{Matroids \& testing in families of algebraically dependent and
...
\begin{itemize}
\item All pairwise \citep{shaffer1986modified,weichert2000robuste}
\item Gold standard designs \citep{bauer1998testing,pigeot2003assessing}
...
\begin{itemize}
\item Cost effectiveness \citep{haller2011cost,gutjahr2012multiple}
\item SNP Studies (Hothorn-Hothorn) \citep{hothorn2009order}
...
\begin{proof}
  First assume that $I$ exhaustive, then for every $j \in S \setminus
  I$ $H_{I \cup j} \subsetneq H_{I}$, that is $j \nsim I$ for all $j
  \in S\setminus I$ and therefore $I$ is flat.  

  Next, consider $I \subseteq S$ and $I \in \mathscr{L}$, that is
  $\sigma I = I$ assume that $I$ is not exhaustive, then $\exists J
  \supersetneq I$ with $H_I \subset H_J$. Therefore, $\exists j \in J
  \setminus I$ such that $H_I \in H_{J \setminus j}$ and therefore
  also $H_I \subseteq H_j$. Consequently $H_{I \cup j} = H_{I}$, that
  is $j \sim I$, which contradicts that $I$ is a flat and completes
  the proof.
\end{proof}

To get a result on strictly exhaustive set we need to restrict
condition (\ref{cond:matroida}) further. {\sl It is an open question
  whether one can find conditions that make it easier to show matroid
  properties}. 
...
\citep{avis1992pivoting,avis1996reverse,ferrez2001cuts} which are
polynomial in time and input size and compact in space!


\bibliographystyle{alpha}
\bibliography{cleanrefs}
