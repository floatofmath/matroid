...
\begin{proof}
  ...
  r+k \in E\}$ are rejected by the stepwise procedure. It remains to
  be shown that the stepwise procedure also rejects $H_E$ for all
  exhaustive sets with $\min E < r+k$. Note that $\{E \in \mathscr{L}:
  \min E < r+k\} = \overline{\underline{E}}_1^r \cup
  ...
  We show by contradiction that we have $i \in \sigma \{r+l,r+k\}$ for
  some $i \leq r$ and all $1 \leq l < k$.  Assume that there is an $l$
  such that there is no $i \leq r$ with $i \in \sigma
  \{r+l,r+k\}$. Consequently for all $i \leq r$, we have $i \nsim
  \{r+l,r+k\}$. Since $M(\mathscr{H})$ has no loops or parallels also
  $(r+l) \nsim (r+k)$. Consequently, for any maximally independent set
  $I \subset \{1,...,r\}$, $I \cup \{r+l, r+k\}$ is independent and
  has cardinality $\rho\{1,...,r\}+2$ which contradicts
  (\ref{eq:cond.hierarchical}).

  Above argument can be repeated for any set $I$ with $r+1 \leq \min I
  < k$. Therefore, we have $i \in E$ for all $E \in
  \overline{\underline{E}}^{r+k-1}_{r+1}$. Rejection of these sets
