...
exportUDgraph <- function(graph, file = NULL,package="TikZ", scale=1, alpha=0.05, pvalues,
		fontsize=c("tiny","scriptsize", "footnotesize", "small",
		"normalsize", "large", "Large", "LARGE", "huge", "Huge"),
		nodeTikZ, labelTikZ="near start,above,fill=blue!20",
		tikzEnv=TRUE, offset=c(0,0),fill=list(reject="red!80",retain="white"), nodeR=25) {
	...
	for (node in getNodes(graph)) {
		...
		y <- getYCoordinates(graph, node) + nodeR # It is more important to use nodeR to reduce the arc that should be drawn.
                ...
                ### our graphs don't need alphas nor weights
	...
	## } else {
        ...
        for (i in getNodes(graph)) {
          for (j in getNodes(graph)) {			
            if (graph@m[i,j]!=0) {
              ### We don't need edge weights (for the moment)
              ##
              #weight <- getWeightStr(graph, i, j, LaTeX=TRUE)
              ## But we can encode the hypothesis using weights 
              weight <- paste("H_{",getWeightStr(graph, i, j),"}",sep="")
	...
	if (!missing(fontsize)) {
        ...
        if(!is.null(file)){
          cat(LaTeXHeader(),file=file)
          cat(tikz,file=file,append=T)
          cat("\\end{document}",file=file,append=T)
          return(NULL)
        } else {
          return(tikz)
        }
